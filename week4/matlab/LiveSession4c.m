...
%--------------------------------------------------------------------------
% Set lower and upper bounds for the number of hidden nodes
lb = [1,1];
ub = [100,5];
...
opts = optimoptions(...
    'ga',...
    'PlotFcn',{@gaplotbestf,@gaplotgenealogy,@gaplotscorediversity},...
    'MaxGenerations',5,...
    'Display','iter',...
    'UseParallel',true...
    );
...
% Do an optimization to find the best number of hidden nodes
% Call the genetic algorithm (ga) solver where x(1) has integer values
rng(1,'twister') % for reproducibility
IntCon = [1,2];
[best_values,fval,exitflag] = ga(@scalarobjective,nvars,[],[],[],[],...
    lb,ub,[],IntCon,opts)

%--------------------------------------------------------------------------
% now do the fit using this number of hidden nodes
...
% Create a Fitting Network with best_hiddenLayerSize nodes in the hidden layer
best_hiddenLayerSize=best_values(1);
net = fitnet(best_hiddenLayerSize,trainFcn);

% Set transfer function
itransferFunctions=best_values(2);
...
set(gca,'FontSize',16)
set(gca,'LineWidth',2);  
print('best-nn-errhist-m.png','-dpng')
...
set(gca,'FontSize',16)
set(gca,'LineWidth',2);  
print('best-nn-scatter-m.png','-dpng')
